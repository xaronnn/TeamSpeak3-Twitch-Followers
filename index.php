<title>XARON TWITCH BOT</title>

<?php
/*
 Author: XARON
 File: TeamSpeak3TwitchFollowers: Main
 Last Update: 19.08.2017 21:49

 I'm selling private scripts. Contact me on facebook;
...
$followerPermId = '9'; //permission id

require_once("libraries/TeamSpeak3/TeamSpeak3.php");
$ts3Connect = TeamSpeak3::factory("serverquery://{$username}:{$password}@{$host}:{$queryPort}/?server_port={$serverPort}&blocking=0&nickname={$botName}");
...
echo '<a href="'.$twitchtv->authenticate().'">Give me fucking permission!</a><br/>';
$ttv_code = $_GET['code'];
...
if(isset($user_name)) {
    ...
    curl_setopt_array($ch, array(
        ...
        CURLOPT_URL => $channelsApi . $channelName.'/'.$istek
    ));
    ...
    for($i=0; $i < $decode->_total; $i++){
        $isim = $decode->follows[$i];
        array_push($arr,$isim->user->name);
    }

    if(in_array($user_name, $arr)){
        $clientList = $ts3Connect->clientList(array('connection_client_ip' => $_SERVER['REMOTE_ADDR']));
        
        foreach($clientList as $clientInfo){
            if($clientInfo['client_type'] == 1) continue;
            $ts3Connect->clientGetByUid($clientInfo['client_unique_identifier'])->addServerGroup($followerPermId);
        }
        echo 'thank you for following, you earned follower permission! :)';
    }else{
        echo "fu*k off, you don't following me :'(";
    }
}
echo '</br><pre>Made by <a href="https://fb.com/XARONNN" target="_blank">XARON</a></pre>'
?>